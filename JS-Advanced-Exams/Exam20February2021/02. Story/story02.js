class Story {
    ...
    toString(sortingType) {
        ...
        let sortedComments = this._comments.sort(sortKind[sortingType]);
        sortedComments.forEach(c => c.Replies.sort(sortKind[sortingType]));
        ...
        for (const comment of sortedComments) {
            ...
            commentsStrArr.push(combinedString);
        }
        let fullCommentsStr = this._comments.length > 0 ?
            `\n${commentsStrArr.join('\n')}` :
            '';
        return `Title: ${this.title}
Creator: ${this.creator}
Likes: ${this._likes.length}
Comments:${fullCommentsStr}`;
    }
}
